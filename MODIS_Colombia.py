...
# Working directory
dst = '/home/olivierp/jde_coffee/MODIS/collection6/Colombia'
...
statesBoundFiles = ['/home/olivierp/jde_coffee/data/Colombia/aoi/aoi.shp']
...
pwd = "Michele1950"
# Tiles to download.
tiles = ['h10v07', 'h10v08', 'h10v09']  # ['h28v07']
...
if checkQuality:
    ...
    allNDVI = [os.path.join(dst, 'CO/raw_data', f) for 
               f in os.listdir(os.path.join(dst, 'CO/raw_data')) 
               if f.endswith('NDVI.tif') and '_2005' not in f]
    allNDVI.sort()
    allQuality = [os.path.join(dst, 'CO/raw_data', f) for 
                  f in os.listdir(os.path.join(dst, 'CO/raw_data')) 
                  if f.endswith('Quality.tif') and '_2005' not in f]
    allQuality.sort()
    
    allOut = [f.replace('MODIS/collection6/Colombia/CO/raw_data', 'Temp') 
              for f in allNDVI]
