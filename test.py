...
if __name__ == "__main__":
    dst = '/home/olivierp/jde_coffee/MODIS/collection6/Vietnam/LD'
    inMissing = 'masked_missing'
    outMissing = 'filled_missing'
    yearsMissing = [2007]
    daysMissing = [337]
    
    inputRasters = [os.path.join(dst, inMissing, f) for 
			f in os.listdir(os.path.join(dst, inMissing)) 
			if f.endswith('.tif')]
    ...
    tt = []
    for nc, su in zip(cores, suff):
        t0 = time.time()
        gapFill(rasters=inputRasters, seasons=days, years=years,
                outFolder=os.path.join(dst, outMissing),
                suffix=su, nodata=[-3000], iMax=np.inf,
                subsetSeasons=daysMissing, subsetYears=yearsMissing, subsetMissing=None,
                clipRange=(-2000, 10000), parallel=True, nCores=nc)
        t1 = time.time()
